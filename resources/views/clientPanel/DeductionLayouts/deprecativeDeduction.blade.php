...
@section("content")
<?php
$DepDeductions = DB::table('deduc_depreciation_deduction')->select('*')->where('user_id', Auth::User()->id)->where('tax_year_id', Session::get('tax_year_id'))->get();
...
function DeprecativeDeducDataSave(){
    alert($("#DepDeduForm").serialize());
          $.ajax({
            type: 'get',
            url: '{{ URL::to('DeprecativeDeducDataSave')}}',
